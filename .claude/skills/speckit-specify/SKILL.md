...
- For each remaining hook, do **not** attempt to interpret or evaluate hook `condition` expressions:
  ...
  - If the hook defines a non-empty `condition`, conditional hooks are unsupported by this workflow: skip the hook and omit `EXECUTE_COMMAND`
...
3. **Create the spec feature directory**:
   ...
   **Create the directory and spec file**:
   ...
   - Copy `.specify/templates/spec-template.md` to `SPECIFY_FEATURE_DIRECTORY/spec.md` only when that file does not already exist. If it exists, load and update it in place; preserve the existing file if generation fails.
