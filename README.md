...
Canvas polish (current): top-level grid frozen so expand/collapse never shifts sibling wrappers; edges routed as smoothstep with arrow markers, `imports` label suppressed while it's the only edge type. Small readability pass before Slice 4 (edge aggregation). ✅
