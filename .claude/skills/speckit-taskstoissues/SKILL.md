...
1. For each task in the list, derive the stable marker `<!-- speckit-task:<TASK_ID> -->` from the task ID (for example, `T001`). Include that marker verbatim in every generated issue body.
1. Before creating an issue, search the repository's issues for that exact marker, including closed issues. If one matching issue exists, reuse it: update its title, body, labels, and dependency information to match the current task, and do not create another issue. If multiple matches exist, reuse the oldest matching issue and report the duplicates for manual cleanup; never create a new issue for a task with an existing marker.
1. If no matching marker exists, use the GitHub MCP server to create one issue in the repository represented by the Git remote. Tasks without an existing match follow the normal creation flow.
